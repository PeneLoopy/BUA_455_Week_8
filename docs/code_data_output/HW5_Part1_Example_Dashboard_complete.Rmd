---
title: "HW 5 - Part 1 - Example Netflix TV Dashboard - Completed"
...
### Comparison of Netflix and Amazon Stock Perfomance (Jan. 1, 2013 - Oct. 1, 2022)

```{r Panel 1 - import Netflix and Amazon data, include=FALSE}

# download data from netflix and amazon
getSymbols("AMZN", from = "2013-01-01", to = "2022-10-01")
getSymbols("NFLX", from = "2013-01-01", to = "2022-10-01")
...
```{r Panel 2 - Netflix tv release period bar chart, fig.dim = c(10, 5)}
...
```{r Panel 2 - side panel - summary table}
...
```{r Panel 3 - Netflix tv area plot, fig.dim = c(10, 4)}

# area (proportion) plot code begins here
# plot code is incomplete (see instructions)
(nflx_tv_area_plot <- nflx_tv_plot2 |>
    
  ggplot() +                                        # shows stacked areas (proportions) by category
  geom_area(aes(x = year_added, y = total, fill = min_ageF)) +                              
  theme_classic() +
  scale_x_continuous(breaks=seq(2013, 2021,1)) +   # modifies x axis so each year is shown  (See Instructions)
