...
### Comparison of Netflix and AMC Stock Perfomance (Jan. 1, 2013 - Oct. 1, 2022)

```{r Panel 1 - import Netflix and AMC data, include=FALSE}

# download data from netflix and amazon
getSymbols("AMC", from = "2013-01-01", to = "2023-3-01")
getSymbols("NFLX", from = "2013-01-01", to = "2023-3-01")

```


```{r Panel 1 - plot Netflix and AMC stock data, fig.width=12}
...
hw_grid(nflx_fts, amc_fts, ncol=2)
...
The data sources for these two plots are:
...
### Bar chart showing quantity of Movies created and released in each time period by genre and minimum age of intended audience.

```{r Panel 2 - Netflix movie release period data mgmt, include = F}
...
```{r Panel 2 - Netflix mv release period bar chart, fig.dim = c(10, 5)}
...
# labels axes, titles, caption, and legend
  labs(x="Genre", y="Number of Movies", fill="Min. Age",
       ...
       subtitle="I=International   D=Drama   C=Comedy   Do=Docuseries    K=Kids   A/A=Action/Adventure",
...
The data source for this information is [Kaggle](https://www.kaggle.com/)
  
***

### Number of Netflix Movies added each year from 2013 to 2021 by minimum age of intended audience.

```{r Panel 3 - data mgmt for Netflx movie area plot, include=F}

nflx_mv_plot2 <- nflx_mv |>                           # start with nflx_mv
...
```{r Panel 3 - Netflix movie area plot, fig.dim = c(10, 4)}
